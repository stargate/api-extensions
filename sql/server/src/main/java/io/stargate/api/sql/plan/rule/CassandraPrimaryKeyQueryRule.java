...
import io.stargate.api.sql.plan.rel.CassandraSingleRowQuery;
import io.stargate.api.sql.plan.rel.CassandraSingleRowQuery.Condition;
...
public class CassandraPrimaryKeyQueryRule extends RelRule<RelRule.Config> {

  private static final Config CONFIG =
      Config.EMPTY
          .withDescription(CassandraPrimaryKeyQueryRule.class.getSimpleName())
          .withOperandSupplier(
              b ->
  ...
  public CassandraPrimaryKeyQueryRule() {
    super(CONFIG);
  }

  private List<Condition> conditions(LogicalFilter filter, StargateTable table) {
  ...
  public void onMatch(RelOptRuleCall call) {
    ...
    StargateTable table = scan.getTable().unwrap(StargateTable.class);
    ...
    call.transformTo(
        CassandraSingleRowQuery.create(scan.getCluster(), scan.getTable(), conditions));
  }
}
